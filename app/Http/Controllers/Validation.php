...
class Validation {
    ...
    public static function createZone($input, $new)
	{
        ...
        if (($new and Zone::where('NAME', '=', trim($input["name"]))->count() > 0) OR
        	(!$new and is_numeric(trim($input["channel"])) and Zone::where('NAME', '=', trim($input["name"]))->where('ID', '!=', trim($input["id"]))->count() > 0))
	    ...
	    if (($new and is_numeric(trim($input["channel"])) and Zone::where('RELAY_CHANNEL', '=', trim($input["channel"]))->count() > 0) OR
	    	(!$new and is_numeric(trim($input["channel"])) and Zone::where('RELAY_CHANNEL', '=', trim($input["channel"]))->where('ID', '!=', trim($input["id"]))->count() > 0))
